...
class DQNAgent():
    ...
    def select_action(self, state, epsilon=0.):
        ...
        if np.random.uniform() > epsilon:
            with torch.no_grad():
                return torch.argmax(self.Q(torch.from_numpy(state).to(self.device))).item()
        else:
    ...
    def learn(self, experiences, discount):
        ...
        with torch.no_grad():
            targets = torch.reshape(
                                torch.max(self.Q(next_states), dim=1).values,
                                    rewards.size())
            target = rewards + discount * targets
    ...
    def target_update(self, Q, Q_target, tau):
        ...
        for key in q_state.keys():
            target_state[key] = tau * q_state[key] + (1 - tau) * target_state[key]
...
if __name__ == "__main__":
    ...
    for e in range(args.n_episodes):
        ...
        rewards.append(np.mean(moving_window))
    ...
    for t in range(args.max_esp_len):
        ...
        #plt.axis('off')
        #plt.savefig(f"./frames/frame_{t:05d}.png", bbox_inches="tight")
        #plt.clf()
        done = terminated or truncated 
        state = n_state
        if done:
    ...
    import json
    open("dqn_1_lander_experience_no_target.json","w").write(json.dumps(rewards))
    plt.plot(rewards)
    plt.xlabel("Episodes")
    plt.ylabel("Cumulative Reward")
    plt.title("LunarLander-v2 (Experience Replay + no Target Network)")
    plt.tight_layout()
    plt.savefig("lunarlander_replay_no_target.png")
